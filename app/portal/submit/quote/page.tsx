...
export default function QuoteStep() {
  ...
  const [quote, setQuote] = useState(form.quote);
  const [attribution, setAttribution] = useState(form.quoteAttribution);

  const remaining = CHAR_LIMIT - quote.length;
  const canProceed = quote.trim().length > 0 && quote.length <= CHAR_LIMIT;

  const handleNext = async () => {
    updateForm({ quote, quoteAttribution: attribution });
    await saveStep({
      quote_text: quote,
      quote_attribution: attribution || null,
    });
    router.push('/portal/submit/story');
  };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      ...
      <div className="portal-content">
        <div className="ae-card fade-up">
          ...
          <p className="ae-lead">A funny moment, a teacher quote, or a personal motto. Appears prominently on your yearbook page.</p>
          <div style={{ padding: '20px 24px', background: 'rgba(0,53,102,0.03)', border: '1px solid rgba(0,53,102,0.07)', marginBottom: 32 }}>
            ...
            {[
              ...
              <div key={i} style={{ marginBottom: i < 2 ? 10 : 0 }}>
                ...
                {a && <span style={{ fontSize: 12, color: 'var(--gold)', marginLeft: 8 }}>{a}</span>}
              </div>
            ))}
          </div>
          <div style={{ marginBottom: 24 }}>
            <label className="ae-label">Your Quote <span style={{ color: '#c53030' }}>*</span></label>
            <textarea className="ae-textarea" value={quote} onChange={e => setQuote(e.target.value)}
              placeholder='Enter your memorable quote...' rows={3} maxLength={CHAR_LIMIT}/>
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <input className="ae-input" type="text" value={attribution} onChange={e => setAttribution(e.target.value)}
              placeholder="e.g. Mr Heuston, Mam, Coach Kelly..." maxLength={50}/>
          </div>
          {quote && (
            <div style={{ padding: '28px 32px', background: 'var(--navy-deep)', marginBottom: 36 }}>
              ...
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontStyle: 'italic', fontWeight: 300, color: 'var(--cream)', lineHeight: 1.4, marginBottom: attribution ? 10 : 0 }}>
                &ldquo;{quote}&rdquo;
              </div>
              {attribution && <div style={{ fontSize: 12, color: 'var(--gold)', fontFamily: "'DM Sans', sans-serif" }}>— {attribution}</div>}
            </div>
          )}
          <div className="step-nav">
            ...
            <button className="ae-btn-primary" onClick={handleNext} disabled={!canProceed || saving}>
              <span>{saving ? 'Saving...' : 'Next: Your Story →'}</span>
            </button>
          </div>
        </div>
      </div>
